...
def init_db():
    """Initialize the database with users and admins table"""
...
def register():
    ...
    if 'student_email' in session or 'admin_email' in session:
...
@app.route('/dashboard')
def dashboard():
    """Dashboard - main app area for students"""
...
@app.route('/checkout')
def checkout():
    """Checkout page - requires user login"""
    if 'student_email' not in session:
        return redirect(url_for('login'))
